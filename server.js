...
app.get('/fetchWeather', async (req, res) => {
  try {
    ...
    if (!lat || !lon) {
      const errorData = { error: 'lat ve lon parametreleri gerekli', received: { lat, lon } };
    ...
    logRequest('SOAP', { query: req.query }, { status: 200, type: 'JSON', size: weatherData ? weatherData.toString().length : 0 });
    res.set('Content-Type', 'application/json');
    res.send(weatherData);
  } catch (error) {
    console.error('SOAP endpoint error:', error.message);
    logRequest('SOAP', { query: req.query }, null, error);
    res.status(500).json({ error: 'Weather data could not be fetched', message: error.message });
  }
});
...
app.get('/fetchWeatherJson', async (req, res) => {
  try {
    ...
    logRequest('REST', { query: req.query }, { status: 200, type: 'JSON', dataPoints: weatherData ? Object.keys(weatherData).length : 0 });
...
app.get('/logs', (req, res) => {
  ...
  try {
    const logs = protocol ? logger.filterLogs(protocol, date) : logger.getLastLogs(parseInt(count), date);
    res.json({ timestamp: new Date().toISOString(), filter: { protocol, count, date }, total: logs.length, logs: logs.slice(-parseInt(count) || 10) });
  } catch (error) {
...
app.get('/logs/files', (req, res) => {
  const logger = require('./logger');
  res.json({ timestamp: new Date().toISOString(), logDirectory: logger.logsDir, files: logger.listLogFiles() });
});
...
server.listen(PORT, () => {
  ...
  console.log(`  GET /fetchWeather?lat=38.42&lon=27.14&startDate=2025-12-14&numDays=3 (SOAP->JSON)`);
